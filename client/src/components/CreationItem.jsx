...
const CreationItem = ({item}) => {
  ...
  return (
    <div onClick={()=> setExpanded(!expanded)} className='p-4 max-w-5xl text-sm bg-zinc-900  rounded-lg cursor-pointer'>
        <div className='flex justify-between items-center gap-4'>
            <div>
                <h2 className='text-white'>{item.prompt}</h2>
                <p className='text-gray-300'>{item.type} - {new Date(item.created_at).toLocaleDateString()}</p>
            </div>
